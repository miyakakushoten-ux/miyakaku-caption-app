...
if st.button("✨ AIでキャプションを生成する", type="primary", use_container_width=True):
    if not theme_input_feature:
        ...
        with st.spinner("MIYAKAKU LEATHERの魂を込めた文章を生成中..."):
            ...
            # 「その他」や「モータークラフト」など、選ばれたシリーズによってAIへの指示を切り替える
...
"""
            elif selected_series == "モータークラフト":
                series_context = f"""
【今回焦点を当てるシリーズ】
シリーズ名: {selected_series}
シリーズの特徴: {series_options[selected_series]}
【モータークラフト専用の重要指示】
製作者自身が現役のバイク乗りであり、実際のツーリング環境で自ら使用し、テストと改良を繰り返して品質を追求しているという「実体験に基づく説得力と泥臭い情熱」を文章のトーンに自然に滲ませてください。同じライダー層からの共感と信頼を得られるような言葉選びを意識してください。
"""
            else:
                series_context = f"""
【今回焦点を当てるシリーズ】
...
- フォロワーへの問いかけ（コメント促進）: キャプション本文の最後（ハッシュタグの前）に、読者が思わずコメントしたくなるような【3つの異なる問いかけ案】を以下の形式で出力してください。（ユーザーが後から1つ選ぶため、3つとも出力必須です）
  1. 【ライダーへの問いかけ】: （例：ツーリングルートや季節の装備など、ライダーの共感を誘う質問）
  2. 【クラフトマンへの問いかけ】: （例：仕様の好みや二者択一など、こだわりを刺激する質問）
  3. 【日常への問いかけ】: （例：週末の予定や季節の話題など、ライフスタイルに寄り添う質問）
- ハッシュタグ: あなたは優秀なSNSトレンドアナリストでもあります。Instagramにおける「レザークラフト」「ハンドメイド」「バイクギア」界隈の最新トレンドや検索ランキングを推測・分析し、拡散力の高い王道のビッグワード（例：#レザークラフト #革小物 #ハンドメイド など）と、ターゲットに深く刺さるニッチなワードを組み合わせた「最新トレンドを反映した最強のハッシュタグセット」を10〜15個生成してください。なお、#MIYAKAKULEATHER と #宮覚 は必ず含めてください。
- 固定フレーズ: 全ての一番最後に必ず「※現在、正式オープンへ向けた制作記録をお届けしています。」という一文を独立した行で添えてください。
